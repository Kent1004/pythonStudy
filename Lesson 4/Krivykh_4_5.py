...
def currency_rates(argv):
    ...
    currency_date = response[
                    (response.find('<ValCurs Date=') + 15): (response.find('" name="Foreign Currency Market">'))]
    ...
    if response.find(currency) == -1:
        ...
        '''
        print( f"{Decimal(response[begin_position:end_position].replace(',', '.')).quantize(Decimal('1.00'))}, {currency_date}")
